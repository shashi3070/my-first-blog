...
function getinfo(d){
			...
			for(i=0;i<d["blocks"].length;i++){
				...
				//console.log(type)
				ob={"level":obj['parent'],"type":obj["data"][0]['value']}
				//console.log(ob)
				if(type=="python"){
					console.log('python')
	...
	for(i=0;i<parentid.length;i++){
		...
		if(typeofCompo[parentid[i]]=='python'){
			...
			o={"server":final_map[parentid[i]][0][0],"GroupName":final_map[parentid[i]][0][1],"ProjectName":final_map[parentid[i]][0][2],"Version":final_map[parentid[i]][0][3],"JobName":final_map[parentid[i]][0][4],"Environment":final_map[parentid[i]][0][5]}
